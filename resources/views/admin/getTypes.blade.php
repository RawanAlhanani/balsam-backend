                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">جميع أنواع الأنشطة</h4>
                                            ...
                                            @foreach ($types as $item)
                                                <tr>
                                                    <td>{{$item->nomActivite}}</td>
                                                        ...
                                                        <a href="{{route('EditType', $item->id)}}" class="btn btn-round btn-warning btn-sm">تعديل</a>
                                                        
                                                        <button id="{{$item->id}}" type="button" data-toggle="modal" data-target="#supprimerType" class="vouloirSupprType  btn btn-round btn-danger btn-sm">حذف</button>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
...
<!-- supprimer un type -->
<div class="modal fade text-left" id="supprimerType" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                              <div class="modal-content">
                                ...
                                <div class="modal-body">
                                  <h5>هل تريد فعلا حذف هذا النوع ؟</h5>
                                </div>
...
@section('scripts')
    ...
    $('.vouloirSupprType').on('click', function() {
    ...
    // supprimer un tuteur
     $('.doSupprimer').on('click', function() {
        ...
        $.ajax({
               type:'GET',
               url: "deleteType/"+ id_type,
               dataType:'json',
               success:function(data){

                if(data.Ok == 'Ok'){
                    $('#'+id_type).closest('tr').remove();
                }
               }
           });
    });
