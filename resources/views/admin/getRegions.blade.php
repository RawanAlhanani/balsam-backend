                                    ...
                                    <table class="table table-striped table-bordered dataex-html5-selectors">
                                        ...
                                        <tbody>
                                            @foreach ($regions as $item)
                                                <tr>
                                                    <td>{{$item->nom_region}}</td>

                                                        <td class="Last">
                                                                                                            
                                                        <a href="{{route('EditRegion', $item->id)}}" class="btn btn-round btn-warning btn-sm">تعديل</a>
                                                        
                                                        <button id="{{$item->id}}" type="button" data-toggle="modal" data-target="#supprimerRegion" class="vouloirSupprRegion btn btn-round btn-danger btn-sm">حذف</button>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
...
<div class="modal fade text-left" id="supprimerRegion" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                              <div class="modal-content">
                                ...
                                <div class="modal-body">
                                  <h5>هل تريد فعلا حذف هذه المنطقة ؟</h5>
                                </div>
...
@section('scripts')
    ...
    $('.vouloirSupprRegion').on('click', function() {
        id_reg = $(this).attr("id");
    });
