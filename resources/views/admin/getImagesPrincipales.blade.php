                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">جميع الصور الرئيسية</h4>
                                    ...
                                    <table class="table table-striped table-bordered dataex-html5-selectors">
                                        ...
                                        <tbody>
                                            @foreach ($images as $item)
                                                <tr>
                                                    <td><img width="250" src="{{asset('storage/MesImages'.'/'.$item->nomImage)}}" /></td>

                                                        <td class="Last">
                                                                                                              
                                                        <a href="{{route('EditImagesPrincipales', $item->id)}}" class="btn btn-round btn-warning btn-sm">تعديل</a>
                                                        
                                                        <button id="{{$item->id}}" type="button" data-toggle="modal" data-target="#supprimerImagesPrincipales" class="vouloirSupprImagesPrincipales btn btn-round btn-danger btn-sm">حذف</button>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
...
<!-- supprimer une info -->
<div class="modal fade text-left" id="supprimerImagesPrincipales" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                              <div class="modal-content">
                                ...
                                <div class="modal-body">
                                  <h5>هل تريد فعلا حذف هذه الصورة ؟</h5>
                                </div>
...
var id = "";
    ...
    $('.vouloirSupprImagesPrincipales').on('click', function() {
        id = $(this).attr("id");
    });
    ...
    // supprimer un tuteur
     $('.doSupprimer').on('click', function() {
        $.ajax({
               type:'GET',
               url: "deleteImagesPrincipales/"+id,
               dataType:'json',
