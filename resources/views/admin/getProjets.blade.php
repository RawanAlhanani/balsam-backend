        ...
        <div class="content-body">
            ...
            <section id="file-export">
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">جميع الصفحات ـ مشاريعنا ـ</h4>
                    ...
                    <td class="Last">
                    <button id="{{$item->id}}" type="button" class="voirDetailsProjet btn btn-round btn-primary btn-sm" data-toggle="modal" data-target="#onshow"
                    >تفاصيل</button>
                   
                    <a href="{{route('EditProjet', $item->id)}}" class="btn btn-round btn-warning btn-sm">تعديل</a>
                    
                    <button id="{{$item->id}}" type="button" data-toggle="modal" data-target="#supprimerProejt" class="vouloirSupprProjet btn btn-round btn-danger btn-sm">حذف</button>
                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                ...
                                <!-- image page -->
                                    ...
                                    <fieldset>
                                      <div class="input-group input-group-sm" style="margin-right:80px;">
                                        <img id="projet_image" src="" width="200px">
                                      </div>
                                    </fieldset>
                            ...
                            <div class="form-group row">
                                ...
                                <fieldset>
                                  <div class="input-group input-group-sm">
                                    ...
                                    <textarea readonly="" style="line-height:2; font-size:15px;" class="form-control"  id="description" rows="15" placeholder="محتوى الصفحة"></textarea>
...
<!-- supprimer  -->
<div class="modal fade text-left" id="supprimerProjet" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1" aria-hidden="true">
...
@section('scripts')
    ...
    'use strict';
       ...
       $('.voirDetailsProjet').on('click', function() {
        var id = $(this).attr("id");
    //    $('#onshow').on('show.bs.modal', function() {
            $.ajax({
               type:'GET',
               url: "getDetailsProjet/"+id,
               dataType:'json',
               success:function(data){

                     $('.modal #titre').val(data.projet.titre);       
                     $('.modal #projet_image').attr('src', '{{ URL::asset('storage/MesImages' ) }}' + '/' + data.projet.projet_image) ;   
                     $('.modal #description').text(data.projet.description);      
               },
               error:function(data){
               }
           });
...
var id_pa = "";
    ...
    $('.vouloirSupprProejt').on('click', function() {
        id_pa = $(this).attr("id");
    });
