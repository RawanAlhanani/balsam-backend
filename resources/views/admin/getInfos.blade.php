                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">جميع الأخبار</h4>
                                            ...
                                            @foreach ($infos as $item)
                                                <tr>
                                                    <td>{{$item->titre}}</td>
                                                        ...
                                                        <td class="Last">
                                                        <button id="{{$item->id}}" type="button" class="voirDetailsInfo btn btn-round btn-primary btn-sm" data-toggle="modal" data-target="#onshow"
                                                        >تفاصيل</button>
                                                       
                                                        <a href="{{route('EditInf', $item->id)}}" class="btn btn-round btn-warning btn-sm">تعديل</a>
                                                        
                                                        <button id="{{$item->id}}" type="button" data-toggle="modal" data-target="#supprimerInfo" class="vouloirSupprInfo btn btn-round btn-danger btn-sm">حذف</button>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
...
<div class="modal fade text-left" id="onshow" tabindex="-1" role="dialog" aria-labelledby="myModalLabel21" aria-hidden="true">
                            <div class="modal-dialog" role="document" style="margin: 1.75rem 200px 0 20px !important;">
                              <div class="modal-content" style="width:180% !important;">
                                <div class="modal-header">
                                  <h4 class="modal-title" id="myModalLabel21">تفاصيل الخبر</h4>
                                        ...
                                        <fieldset>
                                          <div class="input-group input-group-sm">
                                            <div class="input-group-prepend">
                                              <span class="input-group-text" id="sizing-addon3">عنوان الخبر</span>
                                            </div>
                                ...
                                <!-- Photo info -->
                                    ...
                                    <fieldset>
                                      <div class="input-group input-group-sm" style="margin-right:80px;">
                                        <img id="image_info" src="" width="300px">
                                      </div>
                                    </fieldset>
                            ...
                            <div class="form-group row">
                                ...
                                <fieldset>
                                  <div class="input-group input-group-sm">
                                    <div class="input-group-prepend">
                                      <span class="input-group-text">محتوى الخبر</span>
                                    </div>
                                
                                    <textarea style="font-size:15px; line-height:2;" readonly="" class="form-control"  id="description" rows="10" placeholder="محتوى الخبر"></textarea>
...
<!-- supprimer une info -->
<div class="modal fade text-left" id="supprimerInfo" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                              <div class="modal-content">
                                ...
                                <div class="modal-body">
                                  <h5>هل تريد فعلا حذف هذا الخبر ؟</h5>
                                </div>
                                <div class="modal-footer">
...
@section('scripts')
    ...
    'use strict';
       ...
       $('.voirDetailsInfo').on('click', function() {
        var id = $(this).attr("id");
    //    $('#onshow').on('show.bs.modal', function() {
            $.ajax({
               type:'GET',
               url: "getDetailsInfo/"+id,
               dataType:'json',
               success:function(data){

                     $('.modal #titre').val(data.info.titre);       
                     $('.modal #image_info').attr('src', '{{ URL::asset('storage/MesImages' ) }}' + '/' + data.info.image_info) ; 
               
                      
                     $('.modal #description').text(data.info.description);      
               },
               error:function(data){
               }
           });
...
var id_info = "";
    ...
    $('.vouloirSupprInfo').on('click', function() {
    ...
    // supprimer un tuteur
     $('.doSupprimer').on('click', function() {
        $.ajax({
               type:'GET',
               url: "deleteInfo/"+id_info,
               dataType:'json',
               success:function(data){

                if(data.Ok == 'Ok'){
                    $('#'+id_info).closest('tr').remove();
                }
               }
           });
    });
