                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">جميع الإختصاصات</h4>
                                            ...
                                            @foreach ($doctors as $item)
                                                <tr>
                                                    <td>{{$item->specialite}}</td>
                                                        ...
                                                        <a href="{{route('EditDoctor', $item->id)}}" class="btn btn-round btn-warning btn-sm">تعديل</a>
                                                        
                                                        <button id="{{$item->id}}" type="button" data-toggle="modal" data-target="#supprimerDoctor" class="vouloirSupprDoctor btn btn-round btn-danger btn-sm">حذف</button>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
...
<!-- supprimer doctor -->
<div class="modal fade text-left" id="supprimerDoctor" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                              <div class="modal-content">
                                ...
                                <div class="modal-body">
                                  <h5>هل تريد فعلا حذف هذا الاختصاص ؟</h5>
                                </div>
...
@section('scripts')
    ...
    $('.vouloirSupprDoctor').on('click', function() {
        id_reg = $(this).attr("id");
    });
    ...
    // supprimer un tuteur
     $('.doSupprimer').on('click', function() {
        $.ajax({
               type:'GET',
               url: "deleteDoctor/"+id_reg,
               dataType:'json',
               success:function(data){

                if(data.Ok == 'Ok'){
                    $('#'+id_reg).closest('tr').remove();
                }
               }
           });
    });
