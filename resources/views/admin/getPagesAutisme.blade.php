                            ...
                            <div class="card-content collapse show table-responsive">
                                <div class="card-body card-dashboard">
                                    <table class="table table-striped table-bordered dataex-html5-selectors">
                                        ...
                                        <tbody>
                                            @foreach ($PagesAutisme as $item)
                                                <tr>
                                                    <td>{{$item->titre}}</td>
                                                                                                   
                    <td class="Last">
                    <button id="{{$item->id}}" type="button" class="voirDetailsPageAutisme btn btn-round btn-primary btn-sm" data-toggle="modal" data-target="#onshow"
                    >تفاصيل</button>
                   
                    <a href="{{route('EditPageAutisme', $item->id)}}" class="btn btn-round btn-warning btn-sm">تعديل</a>
                    
                    <button id="{{$item->id}}" type="button" data-toggle="modal" data-target="#supprimerPageAutisme" class="vouloirSupprPageAutisme btn btn-round btn-danger btn-sm">حذف</button>
                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                            ...
                            <div class="form-group row">
                                ...
                                <fieldset>
                                  <div class="input-group input-group-sm">
                                    ...
                                    <textarea style="font-size:15px; line-height:2;" readonly="" class="form-control"  id="description" rows="10" placeholder="محتوى الصفحة"></textarea>
...
<div class="modal fade text-left" id="supprimerPageAutisme" tabindex="-1" role="dialog" aria-labelledby="myModalLabel1" aria-hidden="true">
...
@section('scripts')
    ...
    'use strict';
       ...
       $('.voirDetailsPageAutisme').on('click', function() {
        var id = $(this).attr("id");
    //    $('#onshow').on('show.bs.modal', function() {
            $.ajax({
               type:'GET',
               url: "getDetailsPageAutisme/"+id,
               dataType:'json',
               success:function(data){

                     $('.modal #titre').val(data.PageAutisme.titre);       
                     $('.modal #page_image').attr('src', '{{ URL::asset('storage/MesImages' ) }}' + '/' + data.PageAutisme.page_image) ;   
                     $('.modal #description').text(data.PageAutisme.description);      
               },
               error:function(data){
               }
           });
...
var id_pa = "";
    ...
    $('.vouloirSupprPageAutisme').on('click', function() {
        id_pa = $(this).attr("id");
    });
